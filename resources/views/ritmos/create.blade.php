        ...
        <form action="{{ route('ritmos.store') }}" method="POST">
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <select name="anio" id="anio" class="form-select">
                        ...
                        <option value="1" {{ old('anio') == 1 ? 'selected' : '' }}>1er Año</option>
                        <option value="2" {{ old('anio') == 2 ? 'selected' : '' }}>2do Año</option>
                        <option value="3" {{ old('anio') == 3 ? 'selected' : '' }}>3er Año</option>
                        <option value="4" {{ old('anio') == 4 ? 'selected' : '' }}>4to Año</option>
                        <option value="5" {{ old('anio') == 5 ? 'selected' : '' }}>5to Año</option>
                        <option value="6" {{ old('anio') == 6 ? 'selected' : '' }}>6to Año</option>
                    </select>
                    @error('anio')
                    <div class="text-danger small mt-1">{{ $message }}</div>
                    @enderror
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select name="tipo" id="tipo" class="form-select">
                        ...
                        <option value="Autor" {{ old('tipo') == 'Autor' ? 'selected' : '' }}>Autor</option>
                        <option value="Ritmo Popular" {{ old('tipo') == 'Ritmo Popular' ? 'selected' : '' }}>Ritmo Popular</option>
                        <option value="Adaptación" {{ old('tipo') == 'Adaptación' ? 'selected' : '' }}>Adaptación</option>
                    </select>
                    @error('tipo')
                    <div class="text-danger small mt-1">{{ $message }}</div>
                    @enderror
            ...
            <div class="mb-3">
                <div class="form-check">
                    <input class="form-check-input" type="checkbox" name="opcional" id="opcional" value="1" {{ old('opcional') ? 'checked' : '' }}>
            ...
            <div class="mb-3" id="anioOpcionalContainer" style="display: none;">
                <label for="anio_opcional" class="form-label">
                    <i class="bi bi-calendar-range me-1"></i>
                    Años Opcionales
                </label>
                <input type="text" name="anio_opcional" id="anio_opcional" value="{{ old('anio_opcional') }}" class="form-control" placeholder="Ej: 1er o 2do año, 3er o 4to año">
...
<script>
    ...
    // Mostrar si ya estaba marcado al cargar
    if (document.getElementById('opcional').checked) {
        document.getElementById('anioOpcionalContainer').style.display = 'block';
    }
</script>
@endsection
